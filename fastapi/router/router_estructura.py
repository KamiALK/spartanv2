...
async def get_all_partidos(
    ...
    try:
        if current_user["tipo"] == "admin":
            ...
            return templates.TemplateResponse(
                "user_dos_partidos.html", {"request": request, "partidos": partidos}
            )
        else:
...
async def buscar_partido_by_nombre_equipo(
    equipo: str, partido: PartidoBase, db: Session = Depends(get_db)
):
    if equipo and partido:
        # Llamar a la función para obtener el partido por equipo
        partido_encontrado = function.get_partido_by_equipo(
            db=db, equipo_nombre=equipo, partido=partido
        )
        ...
        if partido_encontrado:
            ...
            return {
                "message": "No se encontró ningún partido para el equipo proporcionado."
            }
    else:
        return {
            "message": "Se requieren tanto el nombre del equipo como los datos del partido para realizar la búsqueda."
        }


@router.post("/get_partidos_campeonato_id")
async def buscar_partidos_by_id_campeonato(
    id_campeonato: int, db: Session = Depends(get_db)
):
    partido_encontrado = function.get_partidos_by_id_campeonato(
        db=db, id_campeonato=id_campeonato
    )
    if partido_encontrado:
        return partido_encontrado
    else:
        return {
            "message": "No se encontró ningún partido con el ID del campeonato proporcionado."
        }
...
async def asignacion_arbitro_partido(
    partido_evaluador_data: partido_arbitro_scheme, db: Session = Depends(get_db)
):
    try:
        updated_partido = function.asignar_arbitro_a_partido(
            db=db, partido_arbitro_data=partido_evaluador_data
        )
...
async def obtener_partidos_asignados(
    ...
    partidos: List[partido_arbitro_scheme] = function.buscar_partidos_asignados(
        db=db, arbitro_id=current_user["ID"]
    )
    data = []
    ...
    return templates.TemplateResponse(
        "user_asignaciones.html", {"request": request, "partidos": data}
    )
...
async def obtener_all_asignaciones(
    ...
    return templates.TemplateResponse(
        "user_dos_asignaciones.html", {"request": request, "partidos": data}
    )
...
async def partidos_asignados(
    ...
    try:
        ...
        partidos_asignados = function.buscar_partidos_asignados(
            db=db, arbitro_id=persona_id
        )
        ...
        for asignacion_partido in partidos_asignados:
            ...
            for p in partido:
                ...
                nombre_equipo_local = function.get_equipo_by_id_equipo(
                    db=db, id_partido=id_local
                )
                nombre_equipo_visitante = function.get_equipo_by_id_equipo(
                    db=db, id_partido=id_visitante
                )
        ...
        return templates.TemplateResponse(
            "user_partidos.html", {"request": request, "partidos": partidos_detalles}
        )
    except:
        partidos = "no hay partidos"
        # print(partidos)
        return templates.TemplateResponse(
            "user_partidos.html", {"request": request, "partidos": partidos}
        )
...
async def create_equipo(equipo: EquipoSchema, db=Depends(get_db)):
    ...
    if created_equipo is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Partido ya existe"
        )
...
async def get_campeonatos(
    request: Request, db=Depends(get_db), current_user=Depends(functio.get_current_user)
):
    if current_user["tipo"] == "Evaluadores":
        ...
        print(campeonatos)
        return templates.TemplateResponse(
            "user_dos_campeonatos.html",
            {"request": request, "campeonatos": campeonatos},
        )
    else:
...
async def create_evaluacion_arbitro(
    evaluacion_data: EvaluacionesBase, db: Session = Depends(get_db)
):
    # Crear la evaluación
    ...
    if created_evaluacion is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Evaluación ya existe"
        )
...
async def grafica(
    ...
    try:
        ...
        evaluaciones: dict = function.buscar_evaluacion_id_arbitro(db=db, id=number_id)

        return templates.TemplateResponse(
            "user_grafica.html", {"request": request, "data": evaluaciones}
        )
    except:
        return templates.TemplateResponse(
            "user_grafica.html", {"request": request, "data": evaluaciones}
...
#     try:
#         # Obtener el ID del usuario
#         user_id = function.get_current_user(token=token)["ID"]
#
#         # Buscar la evaluación
#         evaluaciones: dict = function.buscar_evaluacion_id(db=db, id=int(user_id))
#
#         if not evaluaciones:
#             # Si no se encontraron evaluaciones, puedes manejarlo de alguna manera, como devolver un mensaje de error o una página de error
#             return templates.TemplateResponse(
#                 "error.html",
#                 {
#                     "request": request,
...
#
#     except Exception as e:
#         # En caso de cualquier otro error, manejarlo aquí
#         return templates.TemplateResponse(
#             "error.html",
#             {
#                 "request": request,
